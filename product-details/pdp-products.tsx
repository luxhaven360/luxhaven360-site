...
const LuxuryPDP: React.FC = () => {
  ...
  async function loadItem() {
    setLoading(true);
    try {
      // Percorso assoluto corretto per GitHub Pages
      const resp = await fetch('https://luxhaven360.github.io/luxhaven360-site/products/shop.json', { cache: 'no-cache' });
      if (!resp.ok) {
        console.error('Impossibile caricare shop.json', resp.status);
        setItem(null);
        setLoading(false);
        return;
      }
      const data: ShopItem[] = await resp.json();
      const found = data.find(d => d.sku === skuParam);
      if (found) {
        setItem(found);
        if (found.images?.length) setSelectedImage(0);
