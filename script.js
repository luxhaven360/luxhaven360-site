...
// Loader logic: mostra per 3 secondi, poi fade out
(function() {
  const loader = document.getElementById('siteLoader');
  if (!loader) return;

  // Forza presenza per il primo paint (evita che il browser salti la transizione)
  loader.classList.remove('hidden');

  // Duration display: 3000ms visible, poi fade
  const VISIBLE_MS = 3000;
  const FADE_MS = 600; // deve coincidere con CSS transition

  // In caso di pagine cached (bfcache), assicuriamoci di ri-applicare l'effetto
  window.addEventListener('pageshow', (ev) => {
    // Se persisted (back/forward cache), rifacciamo l'animazione
    if (ev.persisted) {
      showThenHide();
    }
  });

  // Funzione che mostra e poi nasconde con fade
  function showThenHide() {
    ...
    // Dopo VISIBLE_MS avvia fade out
    setTimeout(() => {
      loader.classList.add('hidden');
      // come fallback, rimuoviamo dal DOM dopo la transizione per evitare overlay residuale
      setTimeout(() => {
        try {
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    // garantiamo piccolo delay per fare il paint iniziale
    setTimeout(showThenHide, 40);
  } else {
    window.addEventListener('DOMContentLoaded', () => {
      setTimeout(showThenHide, 40);
    });
